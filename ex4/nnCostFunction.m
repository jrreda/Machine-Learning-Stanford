...
h0 = a3; 							% m x num_labels == 5000 x 10
...
Y = (1:num_labels)==y; 				% m x num_labels == 5000 x 10 (10 Classes)
  
% Costfunction Without regularization
J = (1/m) * sum(sum((-Y.*log(h0)) - ((1-Y).*log(1-h0))));  									%scalar
  
% Costfunction With regularization 
penalize = (lambda/(2*m)) * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));  %scalar
J = J + penalize; 																			%scalar
...
Z3 = A2 * Theta2';  				% m x num_labels == 5000 x 10, Output has no bias unit.
A3 = sigmoid(Z3);					% m x num_labels == 5000 x 10
...
DELTA2 = DELTA2(:,2:end); 			% 5000 x 25 
									% Removing DELTA2 for bias node (1st column)
...
% Calculating gradients for the regularization - Backpropogation
Theta1_grad_penalize = (lambda/m) * [zeros(size(Theta1, 1), 1) Theta1(:,2:end)];		 	% 25 x 401
Theta2_grad_penalize = (lambda/m) * [zeros(size(Theta2, 1), 1) Theta2(:,2:end)];			% 10 x 26
  
% Adding regularization term to earlier calculated Theta_grad
Theta1_grad = Theta1_grad + Theta1_grad_penalize;
Theta2_grad = Theta2_grad + Theta2_grad_penalize;
